...
§52-3 の公開ゲートは現在マスター全体を単位としており、無関係なマスターの未承認で全ツールの公開が止まる。本表をゲートの絞り込みに使う。
...
エンジンが「課税期間開始日 ≤ `effective_to`」で判定すれば結果は正しくなるが、それは `effective_to` の一般的な意味と違う読み方であり、実装者が知らなければ「課税期間終了日」や「取引日」で比較して誤る。マスター側で `applies_to_period_start_to: "2026-09-30"` を設定するのが本来の形と考える。マスターの修正を伴うため本書では決めない。
