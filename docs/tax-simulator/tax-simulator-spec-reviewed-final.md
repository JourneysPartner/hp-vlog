...
type MunicipalityCode = string & { readonly __jisX0402: unique symbol }; // 全国地方公共団体コード5桁（検査数字を含む）
...
- 2割特例の適用対象課税期間の終期がマスターに無い。適用対象期間を `invoice_transition_master` とは別レコードとしてマスター化し、未登録の課税期間について `eligible` を返さない。
