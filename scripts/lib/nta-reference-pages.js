...
const NTA_REFERENCE_PAGES = {
  // 2026-09-01 に原文（PDF）で確認。
  // インボイスの登録をやめる話は、タックスアンサーにも基本通達にも十分な記載がなく、
  // 「登録をやめれば免税事業者に戻れる」と単純化した記事が出た（2026-09-01）。
  // 実際は登録した経路（経過措置か課税事業者選択か）で結論が変わる。
  invoice_registration_cancel: {
    label: 'インボイス登録の取消しと免税事業者に戻れる時期',
    title: '国税庁 インボイス制度に関するQ&A 問7・問17',
    url: 'https://www.nta.go.jp/taxes/shiraberu/zeimokubetsu/shohi/keigenzeiritsu/qa_invoice_mokuji.htm',
    verified_at: '2026-09-01',
    notes: [
      '大前提（Q&A問17の原文）: 「適格請求書発行事業者は、その基準期間における課税売上高が'
        + '1,000万円以下となった場合でも免税事業者となりません（消法9①、基通1-4-1の2）。」'
        + '登録している限り、売上が1,000万円以下でも納税義務は消えない。',
      '免税事業者に戻るための手続（消基通1-4-1の2の原文）: 「その適用を受けようとする'
        + '課税期間の初日から起算して15日前の日までに、…適格請求書発行事業者の登録の'
        + '取消しを求める旨の届出書の提出が必要となる。」'
        + '個人事業者が翌年1月1日から免税に戻るには、その年の12月17日までの提出が必要。'
        + '「12月31日まで」ではない。',
      '取消届出書は期限が延びない（タックスアンサーNo.6629の注6）: 「提出期限等が課税期間の'
        + '初日の前日等までとされている届出書及び適格請求書発行事業者の登録の取消しを求める旨の'
        + '届出書については、該当日が日曜日等の国民の休日等に当たる場合であっても、'
        + 'その日までに提出がなければそれぞれの規定の適用を受けることができません。」',
      '登録した経路で結論が変わる①（経過措置で登録した場合。Q&A問7の原文）: '
        + '「この経過措置の適用を受ける登録日の属する課税期間が令和5年10月1日を含まない場合は、'
        + '登録日の属する課税期間の翌課税期間から登録日以後2年を経過する日の属する課税期間までの'
        + '各課税期間については免税事業者となることはできません（28年改正法附則44⑤）。」'
        + 'いわゆる2年縛り。取消届出書を出しても、この期間は免税事業者に戻れない。',
      '経過措置とは（Q&A問7の原文）: 「令和5年10月1日から令和11年9月30日までの日の属する'
        + '課税期間中において、令和5年10月1日後に登録を受ける場合には、適格請求書発行事業者の'
        + '登録申請書に登録希望日（提出日から15日以降の登録を受ける日として事業者が希望する日）を'
        + '記載することで、その登録希望日から課税事業者となる経過措置が設けられています'
        + '（28年改正法附則44④）。」この場合、課税事業者選択届出書の提出は不要。',
      '2年縛りが適用されない場合（Q&A問7の注1の反対解釈）: 登録日の属する課税期間が'
        + '令和5年10月1日を含む場合は、2年縛りの対象外。',
      '登録した経路で結論が変わる②（課税事業者選択で登録した場合。消基通1-4-1の2の注）: '
        + '「法第9条第4項《課税事業者の選択》の規定により課税事業者を選択している'
        + '適格請求書発行事業者が、同条第1項本文の規定の適用を受けるには、'
        + '法第57条の2第10項第1号に規定する適格請求書発行事業者の登録の取消しを求める旨の'
        + '届出書及び法第9条第5項《課税事業者の選択不適用》に規定する届出書の提出が必要となる。」'
        + '取消届出書だけでは足りず、課税事業者選択不適用届出書も要る。',
      '課税事業者選択の2年縛り（タックスアンサーNo.6501の原文）: 「消費税課税事業者選択届出書を'
        + '提出した事業者は、事業を廃止した場合を除き、原則として、課税事業者となった日から'
        + '2年を経過する日の属する課税期間の初日以後でなければ、消費税課税事業者選択不適用届出書を'
        + '提出することができませんので、課税事業者となった日から2年間は免税事業者となることは'
        + 'できません。」',
    ],
    // 検索語は「インボイス 登録 やめたい」のように語が分かれることがあるため、
    // 連結した語だけでなく単語単位でも拾う。
    match: (t) => kw(t, [
      'インボイス', '適格請求書', '登録の取消', '登録取消', '登録をやめ', 'インボイスをやめ',
  ...
  // ── 法令解釈通達 ───────────────────────────────────────────────
  // 2026-08-20〜21 に、通達に関する誤りが2日続けて出た。
  //   所基通37-14 を「按分が必要」と書いた（実際は継続適用を条件とする任意の取扱い）
  //   商品券の「発行」を非課税と書いた（実際は不課税。消基通6-4-5）
  // いずれもタックスアンサーに書かれていない論点で、LLM が記憶で補っていた。
  // 通達は条番号を間違えても気づきにくいので、原文を渡して照合できるようにする。
  shohi_tsutatsu_bukkin_kitte: {
    label: '物品切手等（商品券・ギフト券・プリペイドカード）の消費税',
    title: '国税庁 消費税法基本通達 第6章第4節 郵便切手類等及び物品切手等の譲渡関係',
    url: 'https://www.nta.go.jp/law/tsutatsu/kihon/shohi/06/04.htm',
    verified_at: '2026-08-21',
    notes: [
      '6-4-5（物品切手等の発行）原文: 「事業者が、法別表第二第4号ハ《物品切手等の譲渡》に'
        + '規定する物品切手等を発行し、交付した場合において、その交付に係る相手先から収受する'
        + '金品は、資産の譲渡等の対価に該当しない。」'
        + '→ 自ら発行して交付する行為は<strong>不課税（課税対象外）</strong>であり、非課税ではない。',
      '譲渡（販売）は非課税。根拠はタックスアンサー No.6229「商品券やプリペイドカードなど」の'
        + '「プリペイドカードの譲渡は、物品切手等の譲渡として非課税とされています」。'
        + 'No.6229 は「発行」については書いていないので、発行の根拠に引かないこと。',
      '6-4-6（物品切手等の取扱手数料）原文: 「事業者が…物品切手等を譲渡した場合において、'
        + '当該譲渡が他の者からの委託によるものであるときは、当該事業者における物品切手等の譲渡は'
        + '…資産の譲渡に該当しないが、当該譲渡に関して受ける取扱手数料は、課税資産の譲渡等の'
        + '対価に該当することに留意する。」',
      '6-4-4（注）「いわゆるプリペイドカードは、物品切手等に該当する。」',
    ],
    match: (t) => kw(t, ['商品券', 'ギフト券', 'ギフトカード', 'プリペイドカード', '物品切手',
      '回数券', 'チケット', '商品引換券']),
  },
  shotoku_tsutatsu_shihonteki_shishutsu: {
    label: '資本的支出と修繕費の区分',
    title: '国税庁 所得税基本通達 〔資本的支出と修繕費等〕',
    url: 'https://www.nta.go.jp/law/tsutatsu/kihon/shotoku/05/07.htm',
    verified_at: '2026-08-21',
    notes: [
      '37-12（少額又は周期の短い費用の必要経費算入）: (1) 一の修理、改良等のために要した金額が'
        + '20万円に満たない場合 (2) おおむね3年以内の期間を周期として行われることが明らかである場合。',
      '<strong>37-13</strong>（形式基準による修繕費の判定）: 資本的支出か修繕費か明らかでない金額が'
        + '(1) 60万円に満たない場合 (2) その固定資産の前年12月31日における取得価額のおおむね'
        + '10%相当額以下である場合。→ 60万円・10%の形式基準は37-13であって37-14ではない。',
      '<strong>37-14</strong>（資本的支出と修繕費の区分の特例）原文: 「…明らかでない金額'
        + '（37－12、37－12の2、37－13又は37－14の2の適用があるものを除く。）がある場合において、'
        + '<strong>継続して</strong>その金額の30％相当額と…前年12月31日における取得価額の10％相当額'
        + 'とのいずれか少ない金額を修繕費の額とし、残余の額を資本的支出の額として…確定申告を行って'
        + 'いる<strong>ときは、これを認めるものとする</strong>。」'
        + '→ 継続適用を条件に認められる<strong>任意の取扱い</strong>であり、義務ではない。'
        + '「按分が必要」と書かないこと。適用しない場合は個別に判定する。',
      '37-12の2（災害の復旧費用の必要経費算入）もこの節にある。',
    ],
    match: (t) => kw(t, ['修繕費', '資本的支出', '原状回復', '改装', '改修', 'リフォーム',
      '内装工事', '修理']),
  },
};
