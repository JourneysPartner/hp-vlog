...
console.log('');
console.log('=== 10. 年分を説明しすぎない・内訳を書かない（税理士レビュー反映）===');
{
  // 2026-09-03 の指摘:
  //   「いま働いている分は令和8年分（申告は令和9年2月〜3月）にあたり」
  //     → 読者は当然わかっている。堅苦しいだけ
  //   「104万円（基礎控除62万円＋令和8年・令和9年の特例加算42万円）」
  //     → 意味が取れない。内訳は制度側の話で読者は使わない
  const block = buildTaxPeriodBlock(new Date('2026-09-03T04:00:00Z'));
  assert(/年分そのものを説明しない/.test(block), '年分を説明しすぎないよう指示する');
  assert(/さらっと書くだけ/.test(block), '年分はさらっと書くよう指示する');
  assert(/いま働いている分は令和8年分（申告は令和9年2月〜3月）にあたり/.test(block),
    '実際に指摘された言い回しを禁止例として挙げている');

  assert(/読者が使う数字だけを書く/.test(STATIC_RULES), '内訳を書かない方針がある');
  assert(/記事には内訳を書かない/.test(STATIC_RULES), '基礎控除の内訳を書かないよう指示する');
  assert(/104万円（基礎控除62万円＋令和8年・令和9年の特例加算42万円）/.test(STATIC_RULES),
    '実際に指摘された書き方を禁止例として挙げている');
  assert(/合計所得金額489万円以下の方で104万円/.test(STATIC_RULES),
    '区分を添える書き方も禁止例として挙げている');
}
