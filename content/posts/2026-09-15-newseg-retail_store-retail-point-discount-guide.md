...
review_warning: ""
summary: "ポイント値引き・値引き販売の売上は「実際に受け取った金額」が課税標準。値引きは売上対価の返還等として消費税を調整し、帳簿保存が必要。自社ポイントと提携ポイントでは仕訳の起点が異なる。"
review_status: "needs_revision"
review_comment: "「間違い①：値引き後の金額に標準税率10%をかけ算して消費税を計算してしまう\n原因：「税抜き × 10% = 消費税」という計算に慣れているため、税込みの値引き後金額にそのまま10%を掛けてしまうケースがあります。正しくは「税込み金額 × 7.8/110」で課税標準に対する消費税額を逆算します。」→「間違い①：値引き後の金額に標準税率10%をかけ算して消費税を計算してしまう\n原因：「税抜き × 10% = 消費税」という計算に慣れているため、税込みの値引き後金額にそのまま10%を掛けてしまうケースがあります。正しくは「税込み金額 × 10/110」で消費税額を逆算します。」に変更して。\n\n「消費税の計算：税込みの値引き額に 7.8/110（軽減税率は 6.24/108）を乗じて控除税額を算出する」→「消費税の計算：税込みの値引き額に 10/110（軽減税率は 8/108）を乗じて控除税額を算出する」に変更して。"
approved_at: ""
publish_at: ""
published_at: ""
pr_number: ""
...
updated_at: "2026-09-15T16:03:17.491+09:00"
---
...
提携ポイントの会計処理は契約内容によって異なるため、導入時に顧問税理士と処理方針を決めておくことを強くおすすめします。
...
### 間違い①：値引き後の金額に標準税率10%をかけ算して消費税を計算してしまう

<strong>原因：</strong>「税抜き × 10% = 消費税」という計算に慣れているため、税込みの値引き後金額にそのまま10%を掛けてしまうケースがあります。正しくは「税込み金額 × 7.8/110」で課税標準に対する消費税額を逆算します。
...
- <strong>消費税の計算</strong>：税込みの値引き額に 7.8/110（軽減税率は 6.24/108）を乗じて控除税額を算出する
